...
export default async function UsoIaPage() {
  ...
  // Agregado por provedor neste mês — só conta o que a SAF pagou (paid_by_customer=false)
  ...
  // Total que clientes pagaram do próprio bolso via BYOK
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Uso de IA — custo estimado</h1>
          <p className="mt-1 text-sm text-[var(--color-fg-muted)]">
            Use estes números pra calibrar o preço da assinatura. Tudo deste mês corrente.
            Câmbio fixado em USD/BRL = {USD_TO_BRL.toFixed(2)}.
          </p>
        </div>
        <GenerateDemoData />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <Stat
          ...
          tone="expense"
          hint="o que sai do seu bolso"
        />
        <Stat
          label="Chamadas BYOK"
          value={byokCalls.toLocaleString("pt-BR")}
          tone="primary"
          hint={`${byokFamilies} família(s) · custo ${formatBRL(byokCost)} (pelo cliente)`}
        />
        <Stat
          label="Receita p/ família ativa"
          value={`R$ ${BRAND.pricing.monthlyBRL.toFixed(2).replace(".", ",")}`}
          tone="income"
          hint="preço base"
        />
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Custo por provedor neste mês</CardTitle>
          <CardDescription>
            Some o que cada motor consumiu de chamada e tokens.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {byProvider.length === 0 ? (
              ...
              <tbody>
                {byProvider.map((r) => (
                  <tr key={r.provider} className="border-b border-[var(--color-border)] last:border-b-0">
                    ...
                    <td className="num py-2 text-right">{Number(r.calls).toLocaleString("pt-BR")}</td>
                    <td className="num py-2 text-right text-xs text-[var(--color-fg-muted)]">
                      {Number(r.tokensIn ?? 0).toLocaleString("pt-BR")}
                    </td>
                    <td className="num py-2 text-right text-xs text-[var(--color-fg-muted)]">
                      {Number(r.tokensOut ?? 0).toLocaleString("pt-BR")}
                    </td>
                    <td className="num py-2 text-right font-semibold text-[var(--color-expense)]">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Custo por família no mês</CardTitle>
          <CardDescription>
            Top {byFamily.length} famílias. Quando não há chamadas reais ainda, projetamos pelo
            número de transações multiplicado pelo custo médio do provedor da família.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {byFamily.length === 0 ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {byFamily.map((r) => {
                  ...
                  // BYOK: o cliente paga, então o custo da Saf é zero.
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={
                          "num py-2 text-right text-xs font-medium " +
                          (margin > 0
      ...
      <Card>
        <CardHeader>
          <CardTitle>Tabela de preços de referência</CardTitle>
          <CardDescription>
            Atualize <code>apps/web/src/lib/ai-pricing.ts</code> quando os provedores
            mudarem o rate card.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <tbody>
              {pricingTable().map((p) => (
                <tr key={p.provider} className="border-b border-[var(--color-border)] last:border-b-0">
                  ...
                  <td className="num py-2 text-right text-xs">${p.inputPerM.toFixed(3)}</td>
                  <td className="num py-2 text-right text-xs">${p.outputPerM.toFixed(3)}</td>
                  <td className="num py-2 text-right text-xs font-medium">
